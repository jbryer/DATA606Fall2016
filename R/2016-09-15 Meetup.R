...
heights.reorderd <- sample(heights)

heights2 <- rnorm(length(heights), mean=mean(heights), sd=sd(heights))
hist(heights2)

qqnorm(heights2, cex=0.5, main='', axes=F, ylab='Simulated Male heights (in)', pch=19)
axis(1)
axis(2)
abline(mean(heights), sd(heights), col="blue", lwd=1.5)
...
# SAT mean = 1500, sd = 300
sat <- rnorm(100, mean=1500, sd=300)
sat
hist(sat)
